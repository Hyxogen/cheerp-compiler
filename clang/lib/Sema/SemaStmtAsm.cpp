...
StmtResult Sema::ActOnGCCAsmStmt(SourceLocation AsmLoc, bool IsSimple,
  ...
  if (const FunctionDecl *Caller = dyn_cast<FunctionDecl>(CurContext)) {
    if (Caller->hasAttr<AsmJSAttr>() && AsmString->getString().size()) {
  ...
  for (unsigned i = 0; i != NumOutputs; i++) {
    ...
    if(!IsAsmJS && !Context.getTargetInfo().isByteAddressable() && (OutTy->isSpecificBuiltinType(BuiltinType::ULongLong) ||
  ...
  for (unsigned i = NumOutputs, e = NumOutputs + NumInputs; i != e; i++) {
    ...
    if(!IsAsmJS && !Context.getTargetInfo().isByteAddressable() && (InTy->isSpecificBuiltinType(BuiltinType::ULongLong) ||
